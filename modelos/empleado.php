<?php
    ...
    class Empleado{
        ...
        public static function validar($nombre,$correo){
            $listaEmp=[];
            $conexionBD=BD::crearInstancia();
            $sql= $conexionBD->prepare("SELECT * FROM empleados WHERE nombre =? AND correo =? ");
            
            $sql->execute(array($nombre,$correo));

            foreach($sql->fetchAll() as $emple){
                $listaEmp[]= new Empleado($emple['id'],$emple['nombre'],$emple['correo']);
            }
            return $listaEmp;
        }
